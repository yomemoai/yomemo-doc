...
<Steps>
  <ol>
    ...
    <li>
      <h3>Save a Memory</h3>
      Store encrypted memories with optional metadata. Use <code>IdempotentKey</code> for upsert: if the same (handle, idempotent_key) exists, the memory is updated instead of creating a new one.
      
      ```go
      ctx := context.Background()
      
      resp, err := client.Add(ctx, memo.AddOpts{
          Content:       "My coffee machine password is 8888",
          Handle:       "passwords",
          Description:  "Coffee machine credentials",
          IdempotentKey: "coffee-creds", // optional: for upsert
      ...
      fmt.Printf("✅ Memory %s (%s)\n", resp.Action, resp.MemoryID)
      ```
      
      <Aside type="note">
        The content is automatically encrypted using AES-GCM and signed with RSA before being sent to the server. The server only stores the encrypted content. <code>Add</code> returns <code>MemoryResponse</code> with <code>MemoryID</code>, <code>Action</code> (created/updated), and <code>IdempotentKey</code>.
      </Aside>
    </li>
...
#### `Add(ctx context.Context, opts AddOpts) (MemoryResponse, error)`

Adds a new encrypted memory or updates an existing one (upsert when `IdempotentKey` is set). Returns `MemoryResponse` with `MemoryID`, `Action` (`created` or `updated`), and `IdempotentKey`.
...
- `opts`: `AddOpts` struct containing:
  ...
  - `IdempotentKey`: Optional. When set, if a memory with the same (handle, idempotent_key) exists, it is updated; otherwise a new memory is created.
...
Retrieves memories filtered by a specific handle. Content is automatically decrypted. The `handle` is URL-encoded by the SDK before the request.
...
Deletes all memories with a specific handle. The `handle` is URL-encoded by the SDK before the request.
...
func main() {
	...
	// Save a memory (resp.Action is "created" or "updated" when using IdempotentKey)
	resp, err := client.Add(ctx, memo.AddOpts{
		Content:     "My coffee machine password is 8888",
		Handle:      "passwords",
		Description: "Coffee machine credentials",
